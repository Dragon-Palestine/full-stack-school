require('dotenv').config(); // أهم خطوة لتفعيل المتغيرات من .env
...
Table.Teacher.belongsToMany(Table.Subject, { through: Table.Teacher_subject, foreignKey: "teacherId", otherKey: "subjectCode" });
Table.Subject.belongsToMany(Table.Teacher, { through: Table.Teacher_subject, foreignKey: "subjectCode", otherKey: "teacherId" });

Table.Student.belongsToMany(Table.Subject, { through: Table.Registartion, foreignKey: "studentId", otherKey: "subjectCode" });
Table.Subject.belongsToMany(Table.Student, { through: Table.Registartion, foreignKey: "subjectCode", otherKey: "studentId" });
...
sequelize.sync()
  .then(async () => {
    const FoundTheAwoner = await Table.Login.findByPk("loai");
    if (!FoundTheAwoner) {
    ...
    // PORT من Railway أو 3000 افتراضي
